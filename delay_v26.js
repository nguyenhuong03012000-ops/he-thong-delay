...
window.delayModule = (function () {
    ...
    function getLocalFilteredData(group) {
        ...
        const fCarrierStr = getChecked('carrier');
        ...
        return baseData.filter(o => {
            ...
            if (fCarrierStr.length > 0 && !fCarrierStr.includes(o.carrier)) return false;
    ...
    function renderLocalTableOnly(group) {
        ...
        if (tableCard) {
            ...
            if (countSpan) countSpan.textContent = `(Showing ${data.length} orders)`;
        }
    }
