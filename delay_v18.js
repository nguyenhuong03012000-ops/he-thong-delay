...
window.delayModule = (function () {
    ...
    function populateFilterDropdowns() {
        ...
        // Dynamic Table `<select>` tags
        const tagsSet = new Set();
        const statusesSet = new Set();

        _allFetchedOrders.forEach(o => {
            if (o.latest_status) statusesSet.add(o.latest_status);

            let tList = [];
            if (Array.isArray(o.order_tags)) {
                tList = o.order_tags.map(t => typeof t === 'object' ? (t.name || t.text || t.label || String(t)) : String(t));
            } else if (typeof o.order_tags === 'string') {
                tList = o.order_tags.split(/[,●\n|]+/).map(t => t.trim());
            }
            tList.forEach(t => { if (t) tagsSet.add(t); });
        });

        const uniqueStatuses = [...statusesSet].sort();
        const uniqueTags = [...tagsSet].sort();

        // Populate all matching table dropdowns safely reserving current selected filter values
        document.querySelectorAll('.tbl-f-status').forEach(el => {
            const currentVal = el.value;
            el.innerHTML = buildOptions(uniqueStatuses, 'All');
            el.value = currentVal;
        });

        document.querySelectorAll('.tbl-f-tag').forEach(el => {
            const currentVal = el.value;
            el.innerHTML = buildOptions(uniqueTags, 'All');
            el.value = currentVal;
        });
    }
    ...
    function applyFilters() {
        ...
        const fOrder = document.querySelector('.tbl-f-order') ? document.querySelector('.tbl-f-order').value.toLowerCase().trim() : '';
        const fTrack = document.querySelector('.tbl-f-track') ? document.querySelector('.tbl-f-track').value.toLowerCase().trim() : '';
        const fCust = document.querySelector('.tbl-f-cust') ? document.querySelector('.tbl-f-cust').value.toLowerCase().trim() : '';
        const fTag = document.querySelector('.tbl-f-tag') ? document.querySelector('.tbl-f-tag').value.toLowerCase().trim() : '';
        const fStatus = document.querySelector('.tbl-f-status') ? document.querySelector('.tbl-f-status').value.toLowerCase().trim() : '';
        const fReason = document.querySelector('.tbl-f-reason') ? document.querySelector('.tbl-f-reason').value.toLowerCase().trim() : '';
        ...
        let orders = _allFetchedOrders.filter(o => {
            ...
            if (fOrder && !String(o.order_id).toLowerCase().includes(fOrder)) return false;
            if (fTrack && !String(o.tracking_number).toLowerCase().includes(fTrack)) return false;
            if (fCust && !String(o.customer_name).toLowerCase().includes(fCust)) return false;
            if (fStatus && !String(o.latest_status).toLowerCase().includes(fStatus)) return false;

            // Note: Reason is calculated dynamically later, we cannot pre-filter it easily here unless we compute it early.
            // Wait, we compute 'reason' inside processLogic! So fReason must be checked after processLogic!
            // I'll leave fReason out of this pre-filter.

            if (fTag) {
                let tagsStr = Array.isArray(o.order_tags) ? o.order_tags.join(' ') : String(o.order_tags);
                if (!tagsStr.toLowerCase().includes(fTag)) return false;
            }
        ...
        let results = processLogic(orders);

        // Apply post-process Reason filter if populated
        const fReasonPost = document.querySelector('.tbl-f-reason') ? document.querySelector('.tbl-f-reason').value.toLowerCase().trim() : '';
        if (fReasonPost) {
            results.needFollow = results.needFollow.filter(o => String(o.reason).toLowerCase().includes(fReasonPost));
            results.overdue = results.overdue.filter(o => String(o.reason).toLowerCase().includes(fReasonPost));
            results.appointment = results.appointment.filter(o => String(o.reason).toLowerCase().includes(fReasonPost));
        }
    ...
    function renderDashboard(res) {
        ...
        function buildRow(keys, o) {
            ...
            // Action Link: Robust triple-tier fallback to eliminate 404s and guarantee conversation selection
            let customLink;
        ...
        nfBody.innerHTML = res.needFollow.map(o => buildRow(['order_id', 'tracking_number', 'carrier', 'city', 'customer_name', 'all_tags', 'status', 'ship_date', 'days_since_ship', 'days_since_update', 'reason'], o)).join('');

        const ovBody = document.getElementById('delayOverdueTbody');
        ovBody.innerHTML = res.overdue.map(o => buildRow(['order_id', 'tracking_number', 'carrier', 'city', 'customer_name', 'all_tags', 'status', 'ship_date', 'days_since_ship', 'days_since_update', 'reason'], o)).join('');

        const apBody = document.getElementById('delayApptTbody');
        apBody.innerHTML = res.appointment.map(o => buildRow(['order_id', 'tracking_number', 'carrier', 'city', 'customer_name', 'all_tags', 'status', 'ship_date', 'days_since_ship', 'days_since_update', 'reason'], o)).join('');
    ...
    // Bind instantaneous filter events for global select/dates
    ...
    // Advance Binding for dynamic class filters across table headers
    ['tbl-f-order', 'tbl-f-track', 'tbl-f-cust', 'tbl-f-tag', 'tbl-f-status', 'tbl-f-reason'].forEach(cls => {
        const syncAndFilter = (e) => {
            if (e.target && e.target.classList.contains(cls)) {
                // Sync all other inputs of the same class
        ...
        document.addEventListener('input', syncAndFilter);
        document.addEventListener('change', syncAndFilter);
    });
