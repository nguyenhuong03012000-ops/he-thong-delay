...
function autoSenseColumns(rows) {
    ...
    // 2. Sense Address (longest text column usually)
    let maxLen = 0;
    for (let k of keys) {
        const i = parseInt(k.split('_')[1]);
        if (i === idx.phone || i === 0 || i === 1) continue;
        const val = (row[k] || '').toString();
        if (val.length > maxLen) { maxLen = val.length; idx.address = i; }
    }

    // 3. Sense City (from remaining columns)
    for (let k of keys) {
        const i = parseInt(k.split('_')[1]);
        if (i === idx.phone || i === idx.address || i === 0 || i === 1) continue;
        const val = (row[k] || '').toString().toLowerCase();
        if (CITIES_SA.some(c => val === c || val.includes(c))) {
            idx.city = i;
            break;
        }
    }
