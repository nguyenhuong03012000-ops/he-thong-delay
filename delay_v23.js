...
window.delayModule = (function () {
    ...
    function renderDashboard(res) {
        ...
        function populateLocalReasons(group, data) {
            const toolbar = document.querySelector(`.local-toolbar[data-group="${group}"]`);
            if (!toolbar) return;
            const reasonSelect = toolbar.querySelector('.lf-reason');
            if (reasonSelect) {
                const curVal = reasonSelect.value;
                const uniqueReasons = [...new Set(data.map(o => o.reason))].filter(Boolean).sort();
                reasonSelect.innerHTML = `<option value="">All Reasons</option>` + uniqueReasons.map(r => `<option value="${r}">${r}</option>`).join('');
    ...
    function getLocalFilteredData(group) {
        ...
        const toolbar = document.querySelector(`.local-toolbar[data-group="${group}"]`);
        if (!toolbar) return baseData;

        const fTag = toolbar.querySelector('.lf-tag').value.toLowerCase().trim();
        const fStatus = toolbar.querySelector('.lf-status') ? toolbar.querySelector('.lf-status').value.toLowerCase().trim() : '';
        const fReason = toolbar.querySelector('.lf-reason') ? toolbar.querySelector('.lf-reason').value.toLowerCase().trim() : '';
        const fDaysShip = parseInt(toolbar.querySelector('.lf-days-ship').value, 10);
        const fDaysUpd = parseInt(toolbar.querySelector('.lf-days-upd').value, 10);

        return baseData.filter(o => {
            if (fTag && !String(o.all_tags).toLowerCase().includes(fTag)) return false;
    ...
    document.querySelectorAll('.lf-tag, .lf-status, .lf-days-ship, .lf-days-upd, .lf-reason').forEach(el => {
        el.addEventListener('change', () => {
            const group = el.closest('.local-toolbar').getAttribute('data-group');
