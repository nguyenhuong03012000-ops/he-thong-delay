...
window.delayModule = (function () {
    ...
    function renderDashboard(res) {
        ...
        function populateLocalReasons(group, data) {
            const tableElement = document.querySelector(`table[data-group="${group}"]`);
            if (!tableElement) return;
            const reasonSelect = tableElement.querySelector('.lf-reason');
            if (reasonSelect) {
                const curVal = reasonSelect.value;
                const uniqueReasons = [...new Set(data.map(o => o.reason))].filter(Boolean).sort();
                reasonSelect.innerHTML = `<option value="" style="color:black">All Reasons</option>` + uniqueReasons.map(r => `<option value="${r}" style="color:black">${r}</option>`).join('');
                if (uniqueReasons.includes(curVal)) reasonSelect.value = curVal;
            }
        }

        populateLocalReasons('needFollow', res.needFollow);
        populateLocalReasons('overdue', res.overdue);
        populateLocalReasons('appointment', res.appointment);
    ...
    function getLocalFilteredData(group) {
        ...
        const fTag = tableElement.querySelector('.lf-tag') ? tableElement.querySelector('.lf-tag').value.toLowerCase().trim() : '';
        const fStatus = tableElement.querySelector('.lf-status') ? tableElement.querySelector('.lf-status').value.toLowerCase().trim() : '';
        const fReason = tableElement.querySelector('.lf-reason') ? tableElement.querySelector('.lf-reason').value.toLowerCase().trim() : '';
        const fDaysShip = tableElement.querySelector('.lf-days-ship') ? parseInt(tableElement.querySelector('.lf-days-ship').value, 10) : NaN;
        const fDaysUpd = tableElement.querySelector('.lf-days-upd') ? parseInt(tableElement.querySelector('.lf-days-upd').value, 10) : NaN;

        return baseData.filter(o => {
            if (fCustomer && !String(o.customer_name).toLowerCase().includes(fCustomer)) return false;
            let tagsStr = Array.isArray(o.order_tags) ? o.order_tags.join(' ').toLowerCase() : String(o.order_tags).toLowerCase();
            if (fTag && !tagsStr.includes(fTag)) return false;
            if (fStatus && !String(o.status).toLowerCase().includes(fStatus)) return false;
            if (fReason && !String(o.reason).toLowerCase().includes(fReason)) return false;
            if (!isNaN(fDaysShip) && o.days_since_ship < fDaysShip) return false;
            if (!isNaN(fDaysUpd) && o.days_since_update < fDaysUpd) return false;
            return true;
        });
    }
    ...
    function renderLocalTableOnly(group) {
        ...
        tbody.innerHTML = data.map(o => buildRow(['order_id', 'tracking_number', 'carrier', 'city', 'customer_name', 'all_tags', 'status', 'ship_date', 'days_since_ship', 'days_since_update', 'reason'], o)).join('');
    }
    ...
    document.querySelectorAll('.lf-tag, .lf-status, .lf-days-ship, .lf-days-upd, .lf-reason, .tbl-filter-customer, .tbl-filter-tag').forEach(el => {
